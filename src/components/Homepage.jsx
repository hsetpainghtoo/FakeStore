...
import { addItems, selectItem, getItems } from "../services/store.service";
    ...
    <Sheet>
      ...
      <div className="w-full h-full  py-10 flex justify-center items-center flex-wrap gap-16">
        {item?.payload?.map((i) => (
          <Card
            key={i.id}
            className="w-[25%] px-1 hover:shadow-2xl hover:scale-105 duration-200"
          >
            <Link to={`/detail/${i.id}`}>
              {/* <Tooltip
              ...
              <p className="font-normal text-lg text-gray-700 dark:text-gray-400 line-clamp-3">
                {i.description}
              </p>
